...
async def warnings(db: AsyncSession, user_id: str, send_mail: bool = False):
    ...
    # Get last 2 portfolio snapshots (incluye breakdown_by_currency)
    result = await db.execute(
        select(
            PortfolioSnapshot.id,
            PortfolioSnapshot.date,
            PortfolioSnapshot.total_value,
            PortfolioSnapshot.breakdown_by_account,
            PortfolioSnapshot.breakdown_by_currency,
        )
    ...
    if snapshot_today:
        print(f"  snapshot_today:     id={snapshot_today[0]} | date={snapshot_today[1]} | total={snapshot_today[2]} | breakdown_by_currency={snapshot_today[4]}")
    else:
...
async def warnings_account(db: AsyncSession, account_id: uuid.UUID, account_name: str, preferences: tuple, snapshot_yesterday: tuple):
    ...
    if preference_pnl_account is not None:
        ...
        if len(rows) == 2:
            # pnl en la tabla es acumulado -> delta diario = hoy - ayer
            latest_pnl_date = rows[0][1]
            latest_pnl = float(rows[0][0]) - float(rows[1][0])
            print(f"    pnl_account | delta diario: pnl_hoy={rows[0][0]}, pnl_ayer={rows[1][0]}, delta={latest_pnl}, date={latest_pnl_date}")
        elif len(rows) == 1:
            # Solo hay un día de historia: no hay con qué restar.
            # NO usamos el valor acumulado como si fuera diario (eso infla la métrica).
            latest_pnl_date = rows[0][1]
            latest_pnl = None
            print(f"    pnl_account | ⚠️  solo 1 registro histórico (date={latest_pnl_date}), no se puede calcular delta diario, se omite para evitar inflar la métrica")
        else:
        ...
        if latest_pnl is not None:
            if snapshot_yesterday is not None:
                snapshot_id, snapshot_date, total_value, breakdown_by_account, _ = snapshot_yesterday
    ...
    if preference_max_drawdown_account is not None:
        ...
        row = result_dd.first()
        max_drawdown_raw = row[0] if row else None
        max_drawdown_date = row[1] if row else None
        # max_drawdown se guarda en puntos porcentuales (-2.94 == -2.94%), lo pasamos a fracción
        max_drawdown = (float(max_drawdown_raw) / 100) if max_drawdown_raw is not None else None
        supera = max_drawdown is not None and abs(max_drawdown) > float(preference_max_drawdown_account)
...
async def warnings_portfolio(db: AsyncSession, user_id: str, account_ids: list, account_names: dict, preferences: tuple, snapshot_today: tuple):
    ...
    if positions_rows:
        ...
        position_info = {
            row[0]: {
                "asset_id": row[2], "account_id": row[3], "symbol": row[4],
                "name": row[5], "currency": row[6], "quantity": row[1],
            }
        ...
        if preference_pnl_position is not None:
            ...
            for position_id, data in position_data.items():
                ...
                # PnL diario real (no el acumulado de PositionDailyMetric.total_pnl)
                pnl_dollars_today = (float(close_hoy) - float(close_ayer)) * float(quantity)
                pnl_pct = round(pnl_dollars_today / valor_posicion_ayer, 4)
                supera = abs(pnl_pct) > float(preference_pnl_position)
                print(f"    pnl_asset | {symbol}: pnl_dollars_total_pnl(no usado)={pnl_dollars}, close_ayer={close_ayer}, close_hoy={close_hoy}, qty={quantity}, valor_ayer={valor_posicion_ayer:.2f}, pnl_pct={pnl_pct:.4f}, threshold={preference_pnl_position}, supera={supera}")
