...
@router.get("/{asset_id}/prices", response_model=list[AssetPriceRead])
async def list_prices(
    ...
    asset = await asset_repo.get_by_id(db, asset_id)
    if asset is None:
        raise HTTPException(status_code=404, detail="Asset not found")
    return await asset_price_repo.list_range(db, asset.id, date_from, date_to)


@router.post("/{asset_id}/prices", response_model=AssetPriceRead, status_code=201)
